...
if(isset($_POST["add_to_cart"])) //add to cart 값이 세팅이 되어졌을 때//
{
    ...
    if(isset($_SESSION["shopping_cart"]))
    {
        ...
        $item_array_id = array_column($_SESSION["shopping_cart"],"item_name"); //item_name 값에 대한 array 정보들//
...
/*        echo '<br>'."change";*/
        ...
        foreach($_SESSION["shopping_cart"] as $keys => $values)
        {
            ...
            {
                //세션에서 제거한다.
                $_SESSION["shopping_cart"][$keys]['item_quantity']=$_GET['quantity'];
                echo '<script>alert("Required Quantity Changed.")</script>';
                echo '<script>window.location="shoppinglist.php"</script>';
            }
        }
    }
...
<div class="card">
    <h2 align=center>Added List</h2>
    <h4 align=center>Choose Your Price Condition</h4>
    <h4>
    <div align="center">
        <form action="/sendexcelmail.php" method="POST" id="chk_info">
        ...
        if(!empty($_SESSION["shopping_cart"]))
        {
            ...
            foreach($_SESSION["shopping_cart"] as $keys => $values) // 각각의 배열요소들을 모두 참조
            {
                ...
                <tr>
                    ...
                    <td align="center">
                        <form method="get"  id="<?php echo $values["item_name"]?>" action="shoppinglist.php" style="margin-bottom: 0px">
                            <input type="hidden" value="change" name="action">
                            <input type="hidden" value="<?php echo $values["item_name"]?>" name="id">
                            <input type="number" value="<?php echo $values["item_quantity"];?>" min="1" name="quantity">
                            <button type="submit" form="<?php echo $values["item_name"]?>" class="view_btn1" style="width: fit-content">change</button>
                        </form>
                    </td>
                    <td align="center"><?php echo $itemname; ?></td>
                    <td><a href="shoppinglist.php?action=delete&id=<?php echo $values["item_name"]?>"> <button class="view_btn1" style="width: fit-content">remove</button> </a></td>
